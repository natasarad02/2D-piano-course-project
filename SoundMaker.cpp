...
namespace SoundMaker {
	...
	void playSound(std::string filePath, GLFWwindow* window, bool keyHandle, bool isPressed, int key)
	{
		...
		isKeyPressedLongerThan(key, 10);
		if (it != lastSoundTimes.end()) { //&& (isKeyPressedLongerThan(key, 100) || isMousePressedLongerThan(100))) {

				auto timeSinceLastSound = std::chrono::duration_cast<std::chrono::milliseconds>(now - it->second);
				...
				{
					if (timeSinceLastSound.count() < 5000 && isKeyPressedLongerThan(key, 10)) {
						std::cout << "Same sound file '" << filePath << "' is already playing. Please wait 1 second." << std::endl;
						//	currentKeyDuration = 2000;
						return;
					}
		...
		if (wasKeyPressed(key) || (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_1) == GLFW_PRESS))
		{
			ISound* sound = engine->play2D(filePath.c_str(), false, false, true);

			if (keyHandle)
				recordedNotes.push_back({ filePath, currentKeyDuration });
